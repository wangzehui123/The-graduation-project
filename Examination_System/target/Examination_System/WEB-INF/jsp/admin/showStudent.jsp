							...
							<nav style="text-align: center">
								<ul class="pagination">
										...
										<c:when test="${pagingVO.curentPageNo<3}">
											<c:forEach var="i" begin="1" end="5">
												<c:if test="${i<=pagingVO.totalCount}">
													...
													><a href="${pageContext.request.contextPath}/admin/showStudent?page=${i}">${i}</a></li>
											...
											<c:if test="${(pagingVO.curentPageNo>0)&&(pagingVO.curentPageNo <= pagingVO.totalCount)}">
												<c:forEach  var="i" begin="1" end="5">
													<c:if test="${((pagingVO.curentPageNo-3+i)<=pagingVO.totalCount)&&((pagingVO.curentPageNo-3+i)>0) }">
														...
														><a href="${pageContext.request.contextPath}/admin/showStudent?page=${pagingVO.curentPageNo-3+i}">${pagingVO.curentPageNo-3+i }</a></li>
											...
											<c:if test="${(pagingVO.curentPageNo>0)&&(pagingVO.curentPageNo <= pagingVO.totalCount)}">
												<c:forEach  var="i" begin="1" end="5">
													<c:if test="${((pagingVO.totalCount-5+i)<=pagingVO.totalCount)&&((pagingVO.totalCount-5+i)>0) }">
														...
														><a href="${pageContext.request.contextPath}/admin/showStudent?page=${pagingVO.totalCount-5+i }">${pagingVO.totalCount-5+i }</a></li>
									...
									<c:if test="${pagingVO.curentPageNo<pagingVO.totalCount }">
										<li><a href="${pageContext.request.contextPath}/admin/showStudent?page=${pagingVO.curentPageNo+1}">下一页</a></li>
									</c:if>
